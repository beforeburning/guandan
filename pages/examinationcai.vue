...
import multipleOptions from "@/components/multipleOptions-caipan.json";
import singleSelection from "@/components/singleSelection-caipan.json";
...
const submit = () => {
  ...
  PostScore(userData.value.uuid, 2, (successNum * 2).toString()).then(
    async (res) => {
      console.log(res);
      if (!res.success) {
